...
class BrickRenderer(object):
    ...
    def render(self):
        ...
        try:
            ...
            BodyRenderer(self.brick_width, self.brick_length, self.brick_height).render()
    ...
    def _parse_dimensions(self, dimensions):
        ...
        if length < 1 or length > 20:
            raise Exception("dimensions[\"length\"] must be: 1..20")
        ...
        FreeCAD.Console.PrintMessage("Dimensions: {0}x{1}x{2}\n"
                                     .format(self.brick_width, self.brick_length, self.brick_height))
    ...
    def _parse_top_studs(self, top_studs):
        ...
        if self.top_stud_style == TopStudStyle.NONE:
            ...
            length_count = int(top_studs["length_count"])

            width_offset = bool(top_studs["width_offset"])
            length_offset = bool(top_studs["length_offset"])
            ...
            if length_offset and self.brick_length == 1:
                FreeCAD.Console.PrintMessage("top_studs[\"length_offset\"] set to False as "
                                             "dimensions[\"length\"] == 1\n")
                length_offset = False
            ...
            if length_count < 1 or length_count > (self.brick_length - 1 if length_offset else self.brick_length):
                raise Exception("top_studs[\"length_count\"] must be: 1..dimensions[\"length\"] or "
                                "1..dimensions[\"length\"]-1 if top_studs[\"length_offset\"] == true")

            self.top_studs_width_count = width_count
            self.top_studs_length_count = length_count
            self.top_studs_width_offset = width_offset
            self.top_studs_length_offset = length_offset

            FreeCAD.Console.PrintMessage("Top Studs: {0} {1}x{2} {3}{4}\n".format(
                "CLOSED" if self.top_stud_style == TopStudStyle.CLOSED else "OPEN",
                self.top_studs_width_count, self.top_studs_length_count,
                "WIDTH_OFFSET " if self.top_studs_width_offset else "",
                "LENGTH_OFFSET" if self.top_studs_length_offset else ""))
    ...
    def _parse_side_studs(self, side_studs):
        ...
        if style != SideStudStyle.NONE and not left and not right and not front and not back:

            FreeCAD.Console.PrintMessage("side_studs[\"style\"] set to SideStudStyle.NONE as "
                                         "none of Left, Right, Front, Back are True\n")
            style = SideStudStyle.NONE
    ...
    def _parse_pins(self, pins):
        ...
        if style != PinStyle.NONE and not left and not right and not front and not back:
        ...
        if offset and (left or right) and self.brick_length == 1:

            FreeCAD.Console.PrintMessage("pins[\"offset\"] set to False as "
                                         "Left or Right are True and dimensions[\"length\"] == 1\n")
            style = PinStyle.NONE

        if offset and (front or back) and self.brick_width == 1:

            FreeCAD.Console.PrintMessage("pins[\"offset\"] set to False as "
                                         "Front or Back are True and dimensions[\"width\"] == 1\n")
            style = PinStyle.NONE
    ...
    def _parse_holes(self, holes):
        ...
        if offset and self.brick_length == 1:
...
class Dialog:
    ...
    def _construct_dimensions_widgets(self):
        ...
        dimensions_length_label = QtGui.QLabel("Length")
        dimensions_length_label.setAlignment(QtCore.Qt.AlignRight)
        dimensions_length_label.setMinimumWidth(100)
        ...
        dimensions_length_spinbox = QtGui.QSpinBox()
        dimensions_length_spinbox.setMinimum(1)
        dimensions_length_spinbox.setMaximum(20)
        dimensions_length_spinbox.setFont(self.normal_font)
        dimensions_length_spinbox.setMinimumWidth(70)
        ...
        dimensions_length_note_label = QtGui.QLabel(u"ℹ")
        dimensions_length_note_label.setToolTip("1..20 studs")
        dimensions_length_note_label.setFont(self.note_font)
        ...
        dimensions_length_group = QtGui.QWidget()
        dimensions_length_group_layout = QtGui.QHBoxLayout(dimensions_length_group)
        dimensions_length_group_layout.setContentsMargins(0, 0, 0, 0)
        dimensions_length_group_layout.addWidget(dimensions_length_label)
        dimensions_length_group_layout.addWidget(dimensions_length_spinbox)
        dimensions_length_group_layout.addWidget(dimensions_length_note_label)
        dimensions_length_group_layout.addStretch(1)
        dimensions_length_group_layout.setAlignment(dimensions_length_label, QtCore.Qt.AlignVCenter)
        dimensions_length_group_layout.setAlignment(dimensions_length_spinbox, QtCore.Qt.AlignVCenter)
        dimensions_length_group_layout.setAlignment(dimensions_length_note_label, QtCore.Qt.AlignVCenter)
        ...
        dimensions_layout.addWidget(dimensions_length_group)
        ...
        self.brick_length_spinbox = dimensions_length_spinbox
    ...
    def _construct_top_studs_widgets(self):
        ...
        top_studs_length_count_label = QtGui.QLabel("Length Count")
        top_studs_length_count_label.setAlignment(QtCore.Qt.AlignRight)
        top_studs_length_count_label.setMinimumWidth(100)

        top_studs_length_count_spinbox = QtGui.QSpinBox()
        top_studs_length_count_spinbox.setMinimum(1)
        top_studs_length_count_spinbox.setMaximum(20)
        top_studs_length_count_spinbox.setFont(self.normal_font)
        top_studs_length_count_spinbox.setMinimumWidth(70)

        top_studs_length_count_note_label = QtGui.QLabel(u"ℹ")
        top_studs_length_count_note_label.setToolTip("1..dimensions.length\nor\n 1..dimensions.length - 1 if "
                                                     "top_studs.length_half_stud_offset == True")
        top_studs_length_count_note_label.setFont(self.note_font)
        ...
        top_studs_length_offset_label = QtGui.QLabel(u"Length Half Stud Offset")
        top_studs_length_offset_label.setAlignment(QtCore.Qt.AlignRight)

        top_studs_length_offset_checkbox = QtGui.QCheckBox()

        top_studs_length_offset_note_label = QtGui.QLabel(u"ℹ")
        top_studs_length_offset_note_label.setToolTip("forced to False if dimensions.length == 1")
        top_studs_length_offset_note_label.setFont(self.note_font)
        ...
        top_studs_length_group = QtGui.QWidget()
        top_studs_length_group_layout = QtGui.QHBoxLayout(top_studs_length_group)
        top_studs_length_group_layout.setContentsMargins(0, 0, 0, 0)
        top_studs_length_group_layout.addWidget(top_studs_length_count_label)
        top_studs_length_group_layout.addWidget(top_studs_length_count_spinbox)
        top_studs_length_group_layout.addWidget(top_studs_length_count_note_label)
        top_studs_length_group_layout.addSpacing(50)
        top_studs_length_group_layout.addWidget(top_studs_length_offset_label)
        top_studs_length_group_layout.addWidget(top_studs_length_offset_checkbox)
        top_studs_length_group_layout.addSpacing(6)
        top_studs_length_group_layout.addWidget(top_studs_length_offset_note_label)
        top_studs_length_group_layout.addStretch(1)
        top_studs_length_group_layout.setAlignment(top_studs_length_count_label, QtCore.Qt.AlignVCenter)
        top_studs_length_group_layout.setAlignment(top_studs_length_count_spinbox, QtCore.Qt.AlignVCenter)
        top_studs_length_group_layout.setAlignment(top_studs_length_count_note_label, QtCore.Qt.AlignVCenter)
        top_studs_length_group_layout.setAlignment(top_studs_length_offset_label, QtCore.Qt.AlignVCenter)
        top_studs_length_group_layout.setAlignment(top_studs_length_offset_checkbox, QtCore.Qt.AlignVCenter)
        top_studs_length_group_layout.setAlignment(top_studs_length_offset_note_label, QtCore.Qt.AlignVCenter)
    ...
    def _construct_pins_widgets(self):
        ...
        pins_offset_note_label.setToolTip("ignored for left/right if dimensions.length == 1 \n"
                                          "ignored for front/back if dimensions.width == 1")
    ...
    def _construct_holes_widgets(self):
        ...
        holes_offset_note_label.setToolTip("ignored if dimensions.length == 1")
    ...
    def on_ok_clicked(self):
        ...
        dimensions = dict([
            ...
            ("length", self.brick_length_spinbox.value()),
        ...
        top_studs = dict([
            ...
            ("length_count", self.top_studs_length_count_spinbox.value()),
            ("width_offset", self.top_studs_width_offset_checkbox.isChecked()),
            ("length_offset", self.top_studs_length_offset_checkbox.isChecked()),
        ])
